...
if uploaded_file:
    ...
    def somar_setup(lista_ferramentas):
        if not isinstance(lista_ferramentas, str): return 0
        total = 0
        for f in lista_ferramentas.split(','):
            nome = f.strip().lower()
            tempo = df_tempos[df_tempos['nome_ferramenta'].str.lower() == nome]['tempo_montagem'].sum()
            total += tempo
        return total

    df_desenhos['tempo_setup_total'] = df_desenhos['ferramentas_necessarias'].apply(somar_setup)
    ...
    # Ordenação em linha única para garantir sintaxe
    colunas_ordenacao = ['ferramentas_necessarias', 'data de entrega', 'tempo_total_os']
    df_sequenciado = df_final.sort_values
