...
def calcular_fim_normal(ini, mins):
    data = ini
    rest = mins
@@ -44,121 +76,403 @@ def calcular_fim_normal(ini, mins):
            rest = 0
        else:
            rest -= 450
...
if menu == "🚀 Sequenciamento":
    ...
    if up:
        if (
            "df_pcp" not in st.session_state
            or "f_name" not in st.session_state
    ...
    if st.button("💾 Atualizar Banco (Tempos)"):
        dict_t = df_t_ed.to_dict(
            orient="records"
        )
        supabase.table("tabela_tempos").upsert(
            dict_t
        ).execute()
        st.success("Banco de Tempos Atualizado!")
        st.info(
            "GitHub pendente: Token não configurado."
        )
